# table

<p align="center"><img src="./client/src/assets/imgs/table logo full name.png" width="50%"/></p>

Table is a social platform built using the MERN stack (MongoDB, Express, React, Node). The goal is to let users be anonymously open about any personal issues they have while being in a judgement-free zone.

<p align="center"><img src="./client/src/assets/imgs/table-example.png" width="35%"/></p>

Like a forum, users can view other posts (aka tables), reply to them, and/or upvote them to provide support for each other.

<p align="center"><img src="./client/src/assets/imgs/table.gif" width="90%"/></p>
